...
CExpressionArray::CExpressionArray(CSTNode* id, CSTNode* num_of_elem) : CSTNode(NT_EXPRESSION_ARRAY, 2, id ,num_of_elem) {}

CArrayReference::CArrayReference(CSTNode* var_id,CSTNode* array_id, CSTNode* num_of_elem) : CSTNode(NT_ARRAY_REF, 3, var_id , array_id, num_of_elem) {}
CArrayReference::CArrayReference(CSTNode* var_id,CSTNode* array_id, CSTNode* num_of_elem, CSTNode* num_of_elem2) : CSTNode(NT_ARRAY_REF, 4, var_id,array_id, num_of_elem, num_of_elem2) {}

CArray::CArray(CSTNode* array) : CSTNode(NT_ARRAY, 1, array) {}

CElems::CElems(CSTNode* elem) : CSTNode(NT_ELEMS, 1, elem) {}
CElems::CElems(CSTNode* elems, CSTNode* elem) : CSTNode(NT_ELEMS, 2, elems,elem) {}

CElem::CElem(CSTNode* elem) : CSTNode(NT_ELEM, 1, elem) {}
...
CSelectionStatement::CSelectionStatement(CSTNode* arg):CStatement(NT_SELECTIONSTATEMENT,1,arg) {}
...
CSwitchStatement::CSwitchStatement(CSTNode* expr, CSTNode* case_options) : CSTNode(NT_SWITCH_STATEMENT, 2, expr, case_options) {}
CSwitchStatement::CSwitchStatement(CSTNode* expr, CSTNode* case_options,CSTNode* default_option) : CSTNode(NT_SWITCH_STATEMENT, 3, expr, case_options,default_option) {}

CCaseOptions::CCaseOptions(CSTNode* case_num) : CSTNode (NT_CASE_OPTIONS, 1, case_num) {}
CCaseOptions::CCaseOptions(CSTNode* case_num, CSTNode* statements) : CSTNode (NT_CASE_OPTIONS, 2, case_num, statements) {}
CCaseOptions::CCaseOptions(CSTNode* case_options , CSTNode* case_num, CSTNode* statements) : CSTNode (NT_CASE_OPTIONS, 3,case_options , case_num, statements) {}

CDefaultOption::CDefaultOption(CSTNode* statements) : CSTNode (NT_DEFAULT_OPTION , 1 ,  statements) {}
...
CForStatement::CForStatement(CSTNode* body) : CStatement(NT_FORSTATEMENT, 1, body) {}
CForStatement::CForStatement(CSTNode* expr2, CSTNode* expr3, CSTNode* body) : CStatement(NT_FORSTATEMENT, 3, expr2 , expr3 , body) {}
CForStatement::CForStatement(CSTNode* expr1,CSTNode* expr2, CSTNode* expr3, CSTNode* body) : CStatement(NT_FORSTATEMENT, 4, expr1, expr2, expr3, body) {}
...
CExpressionPostFixPlus::CExpressionPostFixPlus(CSTNode* arg1) : CExpression(NT_EXPRESSION_POSTFIX_PLUS, 1, arg1) {}
CExpressionPostFixMinus::CExpressionPostFixMinus(CSTNode* arg1) : CExpression(NT_EXPRESSION_POSTFIX_MINUS, 1, arg1) {}
...
void CExpressionArray::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CArrayReference::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CArray::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CElems::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CElem::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
void CSelectionStatement::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
void CSwitchStatement::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CCaseOptions::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CDefaultOption::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
void CExpressionPostFixPlus::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
void CExpressionPostFixMinus::PrintSyntaxTree(ofstream* dotfile, CSTNode* parent) {
	(*dotfile) << "\"" << parent->GetGraphVizLabel() << "\"->\"" << GetGraphVizLabel() << "\";\n";
	CSTNode::PrintSyntaxTree(dotfile, this);
}
...
double CExpressionArray::Evaluate(CSTNode* parent)
{
	GetChild(0)->Evaluate(this);
	value = value * GetChild(1)->Evaluate(this);
	cout << value << endl;
	if (arrayExists)
	{
...
double CElems::Evaluate(CSTNode* parent)
{
	switch (m_children->size())
	{
	case 1:
	{
		value2 += GetChild(0)->Evaluate(this);
	}
	case 2:
	{
		GetChild(0)->Evaluate(this);
		value2 += GetChild(1)->Evaluate(this);
	}
	cout << value2 << endl;
	return 0;
	}
...
double CExpressionPostFixPlus::Evaluate(CSTNode* parent) {
	return GetChild(0)->Evaluate(this) + 1;
}
double CExpressionPostFixMinus::Evaluate(CSTNode* parent) {
	return GetChild(0)->Evaluate(this) - 1;
}
...
double CForStatement::Evaluate(CSTNode* parent) {
	...
	*/
	...
	case 2:
		...
		break;
	case 3:
		for (; GetChild(0)->Evaluate(this); GetChild(1)->Evaluate(this))
		{
...
double CSwitchStatement::Evaluate(CSTNode* parent)
{
	value = GetChild(0)->Evaluate(this);
	GetChild(1)->Evaluate(this);
	if(result == 0  && m_children->size()==3)
		GetChild(2)->Evaluate(this);
	return 0;
}
double CCaseOptions::Evaluate(CSTNode* parent)
{
	...
	{
		if (GetChild(0)->Evaluate(this) == value)
		{
			GetChild(1)->Evaluate(this);
			if (m_breakVisit)
			{
	...
	{
		GetChild(0)->Evaluate(this);
		if (GetChild(1)->Evaluate(this) == value ||  result == 0) 
		{
			if (m_breakVisit)
			{
				m_breakVisit = false;
...
double CDefaultOption::Evaluate(CSTNode* parent) {
	GetChild(0)->Evaluate(this);
	return 0;
}
